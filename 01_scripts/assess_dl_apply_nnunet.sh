...
conda run -n nnUnet nnUNet_predict -i "$INPUT_DIR" -o "$OUTPUT_DIRECTORY_2D" -t 27 --save_npz --overwrite_existing -m 2d
...
#fi
#conda run -n nnUnet nnUNet_predict -i $INPUT_DIR -o $OUTPUT_DIRECTORY_3D -t 27 --save_npz --overwrite_existing -m 3d_fullres
#
...
##run ensemble
#conda run -n nnUnet nnUNet_ensemble -f $OUTPUT_DIRECTORY_3D $OUTPUT_DIRECTORY_2D -o $OUTPUT_FOLDER_ENSEMBLE -pp $POSTPROCESSING_FILE
